...
export default function Invoices() {
  ...
  const filteredInvoices = invoices.filter(invoice =>
    ...
    invoice.client_detail?.name.toLowerCase().includes(search.toLowerCase())
  )
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <Link to="/invoices/new" className="btn-primary flex items-center gap-2 w-fit">
          <Plus size={20} />
          Nouvelle facture
        </Link>
      </div>
      ...
      <div className="card">
        ...
        {filteredInvoices.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            ...
            {search || statusFilter ? 'Aucune facture trouvée' : 'Aucune facture créée'}
          </div>
        ) : (
                ...
                {filteredInvoices.map((invoice) => (
                  <tr key={invoice.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-3 px-4">
                      <span className="font-medium text-primary-600">{invoice.number}</span>
                    </td>
                    <td className="py-3 px-4 text-gray-800">{invoice.client_detail?.name}</td>
